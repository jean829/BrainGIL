...
class GCNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, gate):
    # def forward(self, x, edge_index, edge_attr):
        x = self.linear(x)
        # edge_embedding = self.bond_encoder(edge_attr)
        ...
        if gate is not None:
            tmp_gate = gate.repeat(deg_inv_sqrt.size(0)//gate.size(0))
            tmp = tmp_gate * deg_inv_sqrt
            # tmp = deg_inv_sqrt[col]* gate
            norm = deg_inv_sqrt[row] * tmp[col]
        else:
            norm = deg_inv_sqrt[row] * deg_inv_sqrt[col]
        # norm = deg_inv_sqrt[row] * deg_inv_sqrt[col]

        return self.propagate(edge_index, x=x, norm=norm) + F_act(x + self.root_emb.weight) * 1./deg.view(-1,1)
...
class GNN_node(torch.nn.Module):
    ...
    """
    def __init__(self, args, num_layer, emb_dim, drop_ratio = 0.5, JK = "last", residual = False, gnn_name = 'gin'):
        '''
            emb_dim (int): node embedding dimensionality
        ...
        self.embedding_h= torch.nn.Linear(args.in_dim, emb_dim)
    ...
    def forward(self, batched_data, gate):
        x, edge_index, edge_attr, batch = batched_data.x, batched_data.edge_index, batched_data.edge_attr, batched_data.batch

        ### computing input node embedding

        # h_list = [self.atom_encoder(x)]
        h_list = [self.embedding_h(x)]
        for layer in range(self.num_layer):

            h = self.convs[layer](h_list[layer], edge_index, edge_attr, gate)
...
class GNN_node_Virtualnode(torch.nn.Module):
    ...
    Output:
        node representations
    """
    def __init__(self, args, num_layer, emb_dim, drop_ratio = 0.5, JK = "last", residual = False, gnn_name = 'gin', atom_encode=False):
        '''
            emb_dim (int): node embedding dimensionality
        '''
        ...
        # AtomEncoder将分子图中的原子特征编码为节点嵌入。
        # 它可能使用各种技术和方法来处理原子特征，例如对原子的元素类型、化学性质、位置等进行编码，以生成节点嵌入表示。
        if self.atom_encode:
            self.atom_encoder = AtomEncoder(emb_dim)
        else:
            # 脑科学而非分子结构，不需要AtomEncoder
            self.embedding_h= torch.nn.Linear(args.in_dim, emb_dim)
