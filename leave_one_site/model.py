...
class GraphEnvAug(torch.nn.Module):
    ...
    def __init__(self, args, num_tasks, num_layer = 5, emb_dim = 300, gnn_type = 'gin', drop_ratio = 0.5, gamma = 0.4, use_linear_predictor=False):
        ...
        if 'virtual' in gnn_type: 
            ...
            rationale_gnn_node = GNN_node(2, emb_dim_rat, JK = jk, drop_ratio = drop_ratio, residual = True, gnn_name = gnn_name)
            self.graph_encoder = GNN_node(num_layer, emb_dim, JK = jk, drop_ratio = drop_ratio, residual = True, gnn_name = gnn_name)
