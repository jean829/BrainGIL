...
def train(args, model, device, loader, optimizers, optimizer_name):
    ...
    for step, batch in enumerate(loader):
        ...
        if batch.x.shape[0] == 1 or batch.batch[-1] == 0:
            ...
            loss += args.fdr*criterion(pred['pred_rep'].to(torch.float32)[is_labeled_rep], target_rep[is_labeled_rep].view(-1,1))
            # # V-Rex
            ...
            loss+=tmp_loss*args.vrex_weight
...
def train_model(args, fold, device, model, optimizers, schedulers, train_loader, val_loader):
    best_epoch = 0
    cnt_wait = 0
    best_valid_perf=0.0
    ...
    for epoch in range(args.epochs):
        ...
        # valid_perf = evaluate(args, model, device, val_loader)
        ...
        if val_acc >=  best_valid_perf  or epoch == 0:
            ...
            torch.save(model.state_dict(), '{}/{}_fold_best_model.pth'.format(args.savepath, fold))
            # os.path.join(args.save_path,str(i)+'.pth')
        else:
