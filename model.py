...
class GraphEnvAug(torch.nn.Module):
    ...
    def __init__(self, args, num_tasks, num_layer = 5, emb_dim = 300, gnn_type = 'gin', drop_ratio = 0.5, gamma = 0.4, use_linear_predictor=False):
        ...
        if 'virtual' in gnn_type: 
            rationale_gnn_node = GNN_node_Virtualnode(args, 2, emb_dim_rat, JK = jk, drop_ratio = drop_ratio, residual = True, gnn_name = gnn_name)
            self.graph_encoder = GNN_node_Virtualnode(args, num_layer, emb_dim, JK = jk, drop_ratio = drop_ratio, residual = True, gnn_name = gnn_name)
        else:
            rationale_gnn_node = GNN_node(args, 2, emb_dim_rat, JK = jk, drop_ratio = drop_ratio, residual = True, gnn_name = gnn_name)
            self.graph_encoder = GNN_node(args, num_layer, emb_dim, JK = jk, drop_ratio = drop_ratio, residual = True, gnn_name = gnn_name)
    ...
    def forward(self, batched_data):
        ...
        h_r, h_env, r_node_num, env_node_num, gate = self.separator(batched_data, h_node, self.gate)
